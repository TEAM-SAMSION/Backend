...
public interface TodoRepository extends JpaRepository<Todo, Long> {
    ...
    @Query(value =
        """
                ...
                join Assign  a on a.register.id=r.id and a.todo=t and a.isDeleted=false
                join Category c on c=t.category and c.categoryStatus='ON'
    ...
    @Query(value =
        """
            select count(t)
                from Todo t
                join Register r on r.userId=:userId and r.todoTeam.id=:todoTeamId
                join Assign  a on a.register.id=r.id and a.completionStatus='COMPLETE' and a.todo=t and a.isDeleted=false
                join Category c on c=t.category and c.categoryStatus='ON'
                where t.scheduledDate = :scheduledDate
            """
    )
